"""Checking for Errors"""
#import necessary modules and download data
...
#create an empty list to hold the dates, high and low temperatures
dates, highs, lows = [], [], []
for row in reader:
    #convert the date from a string
    current_date = datetime.strptime(row[2], '%Y-%m-%d')
    #try-except block to account for missing data
    try:
        low = int(row[4]) #pull the value from row 4 as an integer
        high = int(row[3]) #pull the value from row 3 as an integer
    except ValueError:
        print(f"Missing Date for {current_date}")
    else:
        lows.append(low) #add it to the list 'lows'
...
fig, ax = plt.subplots()
#plot lows in blue, highs in red, set alpha to .5 to lighten the color
